...
// sync node:fs under the home the node:os mock names. A fixed path was
// unwritable for CI's non-root user (EACCES from every install) and left real
// lock directories behind on a root box.
const TEST_HOME = vi.hoisted(() => {
	const nodeFs = require("node:fs") as typeof import("node:fs");
	const nodeOs = require("node:os") as typeof import("node:os");
	const nodePath = require("node:path") as typeof import("node:path");
	return nodeFs.mkdtempSync(
		nodePath.join(nodeOs.tmpdir(), "pi-lens-version-drift-home-"),
	);
});
